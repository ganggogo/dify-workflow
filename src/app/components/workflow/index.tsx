...
export const Workflow: FC<WorkflowProps> = memo(({
  ...
  const controlHeight = useMemo(() => {
    if (!workflowCanvasHeight)
      return '100%'
    const height = workflowCanvasHeight - bottomPanelHeight
    const maxHeight = typeof window === 'undefined' ? workflowCanvasHeight : window.innerHeight
    ...
    if (workflowContainerRef.current) {
      const resizeContainerObserver = new ResizeObserver((entries) => {
        for (const entry of entries) {
          ...
          const boundedBlockSize = typeof window === 'undefined' ? blockSize : Math.min(blockSize, window.innerHeight)
          setWorkflowCanvasWidth(inlineSize)
          setWorkflowCanvasHeight(boundedBlockSize)
        }
      })
